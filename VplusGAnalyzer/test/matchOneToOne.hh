...
#ifndef MATCHONETOONE_HH_
#define MATCHONETOONE_HH_
...
unsigned matchOneToOne(const std::vector<T1>& v1, const std::vector<T2>& v2,
                       ...
                       const double maxMatchingDistance = DBL_MAX)
{
    unsigned nused = 0;
    matchFrom1To2->clear();
    ...
    if (n1)
    {
        ...
        if (n2)
        {
            ...
            std::vector<matchOneToOne_MatchInfo> distanceTable(nmatches);
            std::vector<int> taken2(n2);

            for (unsigned i2=0; i2<n2; ++i2)
                taken2[i2] = 0;

            matchOneToOne_MatchInfo* m;
            for (unsigned i1=0; i1<n1; ++i1)
                for (unsigned i2=0; i2<n2; ++i2)
                {
